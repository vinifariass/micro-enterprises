    ...
    try {
      ...
      for (const item of li.data) {
        ...
        if (match && match.variants && item.quantity) {
          ...
          const v = match.variants.find(
            (vv) =>
              name.includes(vv.sku) ||
              (vv.size && name.includes(vv.size)) ||
              (vv.color && name.includes(vv.color))
          );
          if (v) {
      ...
      console.error("Stripe checkout.session.completed handler failed:", error);
      // Log but do not rethrow to avoid noisy retries in this demo setup.
    }
  }
