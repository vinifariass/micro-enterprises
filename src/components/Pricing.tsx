          ...
          {pricing.map((p) => (
            <div key={p.id} className={`rounded-2xl border ${p.highlight ? 'border-blue-600 shadow-xl' : 'border-gray-200 shadow'} p-6`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
                <a href={wa(p.name)} target="_blank" rel="noopener noreferrer" className={`inline-flex w-full items-center justify-center rounded-lg px-4 py-3 font-semibold ${p.highlight ? 'bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'}`}>
                  Falar no WhatsApp
                </a>
                {p.paymentLink && (
                  <a href={p.paymentLink} target="_blank" rel="noopener noreferrer" className={`inline-flex w-full items-center justify-center rounded-lg px-4 py-3 font-semibold ${p.highlight ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-900 text-white hover:bg-black'}`}>
                    Comprar agora
                  </a>
                )}
              </div>
